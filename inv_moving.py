...
configuration["deviceid"] = 0  # Only supported by Devito>=4.8.2
...
def inv(param, maxiter):
    ...
    def callback(xk):
        mask_file = join(param["mask dir"], param["mask filename"])
        if mask_file[-4:] == ".mat":
    ...
    write_mat_inversion(output_file, result, model, res=res, res_model=res_model)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        param = {
            ...
            "mask pre": "true_source_mask_",  # source mask filename
            ...
            "output folder": join(dir_pre, "inversion/mask_inversions"),
            "output pre": "inv_Ricker20k_recAll_dirs10_river_",
        }
    elif len(sys.argv) == 2:
    ...
    if param["ylims"] is None or param["degrees"] is None:
        for m in range(param["ndata"]):
            print("Inversion with " + str(m) + " th data...")
            ...
            param["mask filename"] = param["mask pre"] + str(m) + ".npy"
            param["data filename"] = param["data pre"] + str(m) + ".npy"
            param["output filename"] = param["output pre"] + str(m) + ".mat"
            inv(param, max_iters)
    else:
        ...
        for deg, inlet in param["degrees"]:
            for ylim in param["ylims"]:
                ...
                for m in range(param["ndata"]):
                    print(
                        "Inversion with " + str(m) + " th data, degree ",
