...
class OpticalFlowCore:
    ...
    def __init__(
        self, shape, dtype=numpy.float32, interpolation_order=3, sigma_blur=0.5
    ):
        """Constructor.
        ...
        ### Misc parameters
        self.sigma_blur = (
            sigma_blur  # gaussian blur sigma before spatial gradient computation
        )
    ...
    def DFD(self, im0, im1, U):
        ...
        im1_warp = ndimage.interpolation.map_coordinates(
            im1,
            map_coords,
...
class Typhoon:
    ...
    def solve(
        ...
        if levels_decomp > levels_max:
            print(
                "[!] too many decomposition levels ({}) requested for given wavelet/shape, set to {}.".format(
        ...
        if (levels_estim is not None) and (levels_estim > levels_decomp):
            print(
                "[!] too many estimation levels ({}) requested for decomposition, set to {}.".format(
        ...
        self.levels_estim = (
            levels_estim if (levels_estim is not None) else self.levels_decomp - 1
        )
        ...
        self.pad_size = tuple(
            (block_size - (s % block_size)) % block_size for s in im0.shape
        )
        # the slice to crop back the original area
        self.crop_slice = tuple(
            slice(None, -p if p else None, None) for p in self.pad_size
        )
        # padd if necessary
        ...
        if (self.core is None) or (self.core.shape != im0.shape):
            self.core = OpticalFlowCore(
                im0.shape,
        ...
        self.C_list = [
            pywt.wavedecn(
                Ui, self.wav, level=self.levels_decomp, mode=self.wav_boundary_condition
            )
            for Ui in U
        ]
        ...
        ### Solve
        print(
            "Decomposition over {} scales of details, {} estimated".format(
        ...
        for level in range(self.levels_estim + 1):
            ...
            # minimize
            C_array, min_value, optim_info = optimize.fmin_l_bfgs_b(
                f_g, C_array.astype(numpy.float64), factr=1000.0, iprint=0
            )
            print(
                "\tl-bfgs completed with status {warnflag} - {nit} iterations, {funcalls} calls".format(
                    **optim_info
                )
            )
            ...
            C_list = (
                pywt.array_to_coeffs(
                    Ci, self.slices[i][: level + 1], output_format="wavedecn"
                )
        ...
        U = tuple(
            pywt.waverecn(Ci, self.wav, mode=self.wav_boundary_condition)[
                self.crop_slice
            ]
    ...
    def solve_fast(
        self, im0, im1, wav="haar", levels_decomp=3, levels_estim=None, U0=None
    ):
    ...
    def solve_pyramid(
        ...
        **kwargs
        ...
        for l in range(levels_pyr):
            ...
            tmp_im = ndimage.gaussian_filter(pyr_im0[0], 2.0 / 3.0)
            pyr_im0.insert(0, ndimage.interpolation.zoom(tmp_im, downscale))
            tmp_im = ndimage.gaussian_filter(pyr_im1[0], 2.0 / 3.0)
...
def RMSE(Ua, Ub):
    ...
    """
    return numpy.sqrt(
        numpy.mean(numpy.sum((numpy.asarray(Ua) - numpy.asarray(Ub)) ** 2, axis=0))
    )
...
if __name__ == "__main__":
    ...
    def demo_particles():
        ...
        ### load data
        im0 = (
            numpy.array(Image.open("demo/run010050000.tif").convert(mode="L")).astype(
                float
            )
            / 255.0
        )
        im1 = (
            numpy.array(Image.open("demo/run010050010.tif").convert(mode="L")).astype(
                float
            )
            / 255.0
        )
        ...
        U1, U2 = typhoon.solve(
            im0, im1, wav="db2", mode="periodization", levels_decomp=3, levels_estim=1
        )
        ...
        dpi = 72.0
        fig, axes = pyplot.subplots(2, 4, figsize=(800.0 / dpi, 450.0 / dpi))
        # images
        for ax, var, label in zip(axes[:, 0], [im0, im1], ["image #0", "image #1"]):
            pi = ax.imshow(
                var, vmin=0.0, vmax=1.0, interpolation="nearest", cmap="gray"
            )
            ax.set_title(label)
            ax.set_xticks([])
            ax.set_yticks([])
        ...
        for ax, var, label in zip(
            ...
            pf = ax.imshow(
                var, vmin=-3.0, vmax=3.0, interpolation="nearest", cmap="RdYlBu_r"
            )
            ax.set_title(label)
            ax.set_xticks([])
            ax.set_yticks([])
        # error maps
        for ax, var, label in zip(
            axes[:, -1], [V1 - U1, V2 - U2], ["abs(error U1)", "abs(error U2)"]
        ):
            ...
            ax.set_yticks([])
        # colormaps
        pyplot.subplots_adjust(bottom=0.25, top=0.94, left=0.05, right=0.95)
        axf = fig.add_axes([1.25 / 8.0, 0.16, 2.5 / 8.0, 0.03])
        pyplot.colorbar(pf, cax=axf, orientation="horizontal")
        axf.set_title("displacement (pixel)", fontdict={"fontsize": "medium"})
        axe = fig.add_axes([4.5 / 8.0, 0.16, 2.5 / 8.0, 0.03])
    ...
    def demo_3dshift(seed=123456789):
        ...
        truth_label = "true shift: ({}) pixel".format(
            ", ".join("{:.2f}".format(s) for s in shift)
        )
        estim_label = "estimated mean displacement: ({}) pixel".format(
            ", ".join("{:.2f}".format(Ui.mean()) for Ui in U)
        )
        dpi = 72.0
        fig, axes = pyplot.subplots(3, 5, figsize=(800.0 / dpi, 450.0 / dpi))
        ...
        for i in range(3):
            ...
            for j, (ax, imj) in enumerate(zip(axes[i, :2], [im0, im1])):
                ax.imshow(
                    imj[slice_mid], interpolation="nearest", cmap="gray", vmin=0, vmax=1
                )
                ax.set_xticks([])
                ax.set_yticks([])
        ...
        pyplot.subplots_adjust(left=0.1, right=0.9, bottom=0.14, top=0.9, hspace=0.3)
        # colorbar
    ...
    def demo_3dvortex(seed=123456789):
        ...
        sigma_list = [1.0, 3.0, 9.0]  # a list of gaussian blur sigma
        ...
        sigma = float(size) / 3.0  # make sure it decays enough near edges
        alpha = float(size)  # amplitude factor
        beta = 3.0  # amplitude max of 3rd component
        x0 = [size_tmp // 2 for _ in range(3)]  # coordinates of center of volume
        SF = alpha * numpy.exp(
            (-1.0 / sigma**2) * ((X1 - x0[0]) ** 2 + (X2 - x0[1]) ** 2)
        )
        ...
        U_truth = (
            G2,
            -G1,
            (X3 - margin) * (beta / float(size)),
        )  # divide by n_adv due to multiple steps
        # displacement coordinates
        n_adv = 30  # number of advection steps
        map_coords = numpy.vstack(
            (X.ravel() for X in (X1, X2, X3))
        )  # for interpolation
        for n, Ui in enumerate(U_truth):
            map_coords[n] -= (1.0 / float(n_adv)) * Ui.ravel()
        # very crude advection
        im1 = im0
        for _ in range(n_adv):
            im1 = ndimage.interpolation.map_coordinates(
                im1, map_coords, order=3, mode="constant"
            ).reshape(im0.shape)
        ...
        if margin > 0:
            ...
            U_truth = tuple(
                Ui[margin:-margin, margin:-margin, margin:-margin] for Ui in U_truth
            )
        ...
        for i in range(3):
            ...
            for j, (ax, Uj) in enumerate(zip(axes[i, 2:], U), 1):
                ...
                ax.set_yticks([])
                ax.set_title("U{} @ mid-ax{}".format(j, i + 1))
        pyplot.subplots_adjust(left=0.1, right=0.9, bottom=0.14, top=0.95, hspace=0.3)
        # colorbar
        cax = fig.add_axes([0.565, 0.05, 0.2, 0.03])
        cb = pyplot.colorbar(pc, cax=cax, orientation="horizontal")
        cb.ax.set_title("displacement (pixel)", fontdict={"fontsize": "medium"})
        # labels
        pyplot.figtext(
    ...
    def main(argv):
        ...
        parser.add_argument("-i0", dest="im0", type=str, default="", help="first image")
        parser.add_argument(
            "-i1", dest="im1", type=str, default="", help="second image"
        )
        parser.add_argument(
            "-w", "--wav", dest="wav", type=str, default=None, help="wavelet name"
        )
        parser.add_argument(
            "-d",
            "--decomp",
        ...
        parser.add_argument(
            "--mode", dest="mode", type=str, default=None, help="signal extension mode"
        )
        parser.add_argument(
            "--fast",
            dest="solve_fast",
        ...
        # misc arguments
        parser.add_argument(
            "--demo", dest="demo_name", type=str, default="", help="name of the demo"
        )
        parser.add_argument(
            "--version",
            dest="print_version",
        ...
        parser.set_defaults(
            verbose=True, print_versions=False, display_result=False, solve_fast=False
        )
        ...
        default_demo = lambda: print(
            "[!] Unkown demo '{}', valid demos: {}".format(
        ...
        if args.im0 and args.im1:
            ...
            ### load images
            im0 = (
                numpy.array(Image.open(args.im0).convert(mode="L")).astype(
                    numpy.float32
                )
                / 255.0
            )
            im1 = (
                numpy.array(Image.open(args.im1).convert(mode="L")).astype(
                    numpy.float32
                )
                / 255.0
            )
            ### Solve problem
            tstart = time.clock()
            (U1, U2), typhoon = Typhoon.solve_pyramid(
                im0,
                im1,
                levels_pyr=args.levels_pyr,
            ...
            if args.display_result:
                dpi = 72.0
                fig, axes = pyplot.subplots(2, 3, figsize=(800.0 / dpi, 600.0 / dpi))
                # images
                for ax, var, label in zip(
                    axes[0, 1:], [im0, im1], ["image #0", "image #1"]
                ):
                    pi = ax.imshow(
                        var, vmin=0.0, vmax=1.0, interpolation="nearest", cmap="gray"
                    )
                ...
                pf = []
                for ax, var, label in zip(
                    axes[1, 1:], [U1, U2], ["estim U1", "estim U2"]
                ):
                ...
                qstep = 8
                ax.quiver(
                    typhoon.core.x[1][::qstep],
                    typhoon.core.x[0][::qstep],
                    U2[::qstep, ::qstep],
                    U1[::qstep, ::qstep],
                    pivot="middle",
                    color="b",
