# Implementation basd on examples.seismic.Model
import numpy as np
...
def initialize_damp(damp, padsizes, spacing, abc_type="damp", isRiver=False):
    """
    ...
    for (nbl, nbr), d in zip(padsizes, damp.dimensions):
        if not isRiver or d is damp.dimensions[-2]:
            ...
            dim_r = SubDimension.right(
                name="abc_%s_r" % d.name, parent=d, thickness=nbr
            )
...
class PhysicalDomain(SubDomain):
    ...
    def __init__(self, so, isRiver=False):
    ...
    def define(self, dimensions):
        ...
        if self.isRiver:
            ...
            map_d = {
                d: (d if d == dimensions[-2] else ("middle", self.so, self.so))
                for d in dimensions
            }
        return map_d
...
class UpperDomain(SubDomain):
    ...
    def __init__(self, so):
        super(UpperDomain, self).__init__()
        self.size = so

    def define(self, dimensions):
        """
        Definition of the upper section of the domain for wrapped indices isRiver.
        """

        return {
            d: (d if not d == dimensions[-1] else ("left", self.size))
            for d in dimensions
        }


class LowerDomain(SubDomain):
    name = "lowerdomain"

    def __init__(self, so):
        super(LowerDomain, self).__init__()
        self.size = so

    def define(self, dimensions):
        """
        Definition of the lower section of the domain for wrapped indices isRiver.
        """

        return {
            d: (d if not d == dimensions[-1] else ("right", self.size))
            for d in dimensions
        }


class FrontDomain(SubDomain):
    name = "frontdomain"

    def __init__(self, so):
        super(FrontDomain, self).__init__()
        self.size = so

    def define(self, dimensions):
        """
        Definition of the front section of the domain for wrapped indices isRiver.
        """

        return {
            d: (d if not d == dimensions[0] else ("left", self.size))
            for d in dimensions
        }


class BackDomain(SubDomain):
    name = "backdomain"

    def __init__(self, so):
        super(BackDomain, self).__init__()
        self.size = so

    def define(self, dimensions):
        """
        Definition of the back section of the domain for wrapped indices isRiver.
        """

        return {
            d: (d if not d == dimensions[0] else ("right", self.size))
            for d in dimensions
        }
...
class GenericModel(object):
    ...
    def __init__(
        ...
        physdomain = PhysicalDomain(space_order, isRiver=isRiver)
        subdomains = subdomains + (physdomain,)
        if isRiver:
            upperdomain = UpperDomain(space_order)
            lowerdomain = LowerDomain(space_order)
            frontdomain = FrontDomain(space_order)
            backdomain = BackDomain(space_order)
            ...
            origin_pml[0] = origin[0]
            shape_pml[-1] -= 2 * self.nbl
            shape_pml[0] -= 2 * self.nbl
        ...
        if callable(bcs):
            ...
            if init or re_init:
                if re_init and not init:
                    ...
                    warning(
                        'Model has to be created with `bcs="%s"`'
    ...
    def padsizes(self):
        ...
        # padsizes.append((0 if self.isRiver else self.nbl, self.nbl))
        padsizes = [(0, 0) for _ in range(self.dim)]
        padsizes[-2] = (self.nbl, self.nbl)  # damp along y-axis
        return padsizes
    ...
    def _gen_phys_param(self, field, name, space_order, is_param=True, default_value=0):
        if field is None:
            return default_value
        if isinstance(field, np.ndarray):
            function = Function(
                name=name, grid=self.grid, space_order=space_order, parameter=is_param
            )
...
class RiverModel(GenericModel):
    ...
    def _initialize_physics(self, vp, space_order, **kwargs):
        ...
        if "vs" in kwargs:
            vs = kwargs.pop("vs")
            self.lam = self._gen_phys_param(
                (vp**2 - 2.0 * vs**2) / b, "lam", space_order, is_param=True
            )
            self.mu = self._gen_phys_param(
                vs**2 / b, "mu", space_order, is_param=True
            )
        else:
    ...
    def _cfl_coeff(self):
        ...
        if "lam" in self._physical_parameters or "vs" in self._physical_parameters:
            coeffs = fd_w(
                1, range(-self.space_order // 2 + 1, self.space_order // 2 + 1), 0.5
            )
    ...
    def critical_dt(self):
        ...
        dt = (
            self._cfl_coeff
            * np.min(self.spacing)
            / (self._thomsen_scale * self._max_vp)
        )
    ...
    def update(self, name, value):
        ...
        if isinstance(value, np.ndarray):
            if value.shape == param.shape:
                ...
                raise ValueError(
                    "Incorrect input size %s for model" % value.shape
                    + " %s without or %s with padding" % (self.shape, param.shape)
                )
        else:
            param.data = value
