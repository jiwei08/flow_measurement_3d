...
% load the two frames
% im1 = im2double(imread('out-00.jpeg'));
% im2 = im2double(imread('out-01.jpeg'));
...
tmp=imflow(:,:,1); tmp(~logical(im1))=256; imflow(:,:,1)=tmp;
tmp=imflow(:,:,2); tmp(~logical(im1))=256; imflow(:,:,2)=tmp;
tmp=imflow(:,:,3); tmp(~logical(im1))=256; imflow(:,:,3)=tmp;
