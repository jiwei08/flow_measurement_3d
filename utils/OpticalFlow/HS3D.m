function [ux, uy, uz] = HS3D(image1, image2, alpha, maxIters)
    % Horn-Schunck method
    % Parameter:
    ...
    for iter = 1:maxIters
        ...
        ux = ux_tmp - (Ix .* (Ix .* ux_tmp + Iy .* uy_tmp + Iz .* uz_tmp + It)) ./ (alpha^2 + Ix.^2 + Iy.^2 + Iz.^2);
        uy = uy_tmp - (Iy .* (Ix .* ux_tmp + Iy .* uy_tmp + Iz .* uz_tmp + It)) ./ (alpha^2 + Ix.^2 + Iy.^2 + Iz.^2);
        uz = uz_tmp - (Iz .* (Ix .* ux_tmp + Iy .* uy_tmp + Iz .* uz_tmp + It)) ./ (alpha^2 + Ix.^2 + Iy.^2 + Iz.^2);
